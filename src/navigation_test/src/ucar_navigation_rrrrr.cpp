...
int isone = 0, get_yaw = 0,need_twice = 0;
...
double goal_angle_J[6] = {0, 0, 0, 0, 0, 0}; // 一个点给到三个角度

double turn_vel = 0.2;
double turn_vel_J = 0.2;
double ctl_vel_J = 0.2;
double turn_vel_line = 0.3;
double vel_x_J = 0.2;
...
nav_msgs::OccupancyGrid global_costmap_data;
...
void init_param_goal()
{
    ...
    n.param<double>("kp_midline_", Kp_midline, 0);
    n.param<double>("kd_midline_", Kd_midline, 0);
    n.param<double>("kp_turnright_", Kp_turnright, 0);
    n.param<double>("kd_turnright_", Kd_turnright, 0);
    n.param<double>("kp_turnleft_", Kp_turnleft, 0);
    n.param<double>("kd_turnleft_", Kd_turnleft, 0);
    // 区域B，第一个停车点
    ...
    n.param<double>("goal_angle_J_5_", goal_angle_J[4], 0);
    n.param<double>("goal_angle_J_6_", goal_angle_J[5], 0);
    n.param<double>("turn_vel_J_", turn_vel_J, 0);
    n.param<double>("turn_vel_", turn_vel, 0);
    n.param<double>("ctl_vel_J_", ctl_vel_J, 0);
    n.param<double>("turn_vel_line_", turn_vel_line, 0);
    n.param<double>("vel_x_J_", vel_x_J, 0);
    ...
    if (use_slam)
        pose_areaA.orientation = setQuaternion(goal_areaA_zs);
    else
...
void pic_CB(const std_msgs::Int8 &pic)
{
    ...
    else
    {
        if (cv_mode.data == 10)
        {
            line_status = MID_LINE;
        }
        else if (cv_mode.data == 12)
        {
            line_status = TURN_RIGHT;
        }
        else if (cv_mode.data == 11)
        {
            line_status = TURN_LEFT;
        }
        else
        {
...
void pic_lineCB(const std_msgs::Float32 &pic)
{
    cv_line_now = pic; 
}
...
{
    double last_cv;
    turn_line = cv_line_now.data * Kp + (cv_line_now.data - last_turn_line) * Kd;
    turn_line = turn_line * 0.8 + 0.2 * last_turn_line;
    last_cv = cv_line_now.data;
    last_turn_line = turn_line;
...
void rotateAndStop(double target_angle)
{
    ...
    double error = my_abs(target_angle - now_yaw);
    
    while (ros::ok() && error > 0.2) // 误差小于0.01时停止
    {
        error = my_abs(target_angle - now_yaw);
        if(error > 1.5)
            error = 1.5;
        fake_vel_now.angular.z = -1.4*error; // 简单的比例控制
        cmd_vel_pub.publish(fake_vel_now);
        ros::spinOnce();
        rate.sleep();
    }
...
int main(int argc, char *argv[])
{
    ...
    cv_line_sub = n.subscribe("/pic_line", 100, pic_lineCB);
    ...
    flag_config_pub = n.advertise<std_msgs::Int32>("/flag1", 100);
    ...
    pub_ClearCostmap = n.advertise<std_msgs::String>("/flag_Clear", 1); // 发布清除costmap的信�???????????
    ...
    while (ros::ok())
    {
        ...
        case status_arrived_areaJ_1:
            ...
            else if (stop_flag == 1) 
            {
                ...
                else
                {
                    ...
                    {
                        ROS_INFO("ABORTED or REJECTED");
                        pose_areaJ_2.position.x = replandeal(pose_areaJ_1.position.x, pose_areaJ_2.position.x);
                        pose_areaJ_2.position.y = replandeal(pose_areaJ_1.position.y, pose_areaJ_2.position.y);
        ...
        case status_arrived_areaA:
            ...
            // shutdownNode("/line_fitting_node");
