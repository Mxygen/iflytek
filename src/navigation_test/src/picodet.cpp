...
PicoDet::PicoDet(const char *param, const char *bin, int input_width,
                 int input_hight, int class_num, bool useGPU)
{ //, float score_threshold_ = 0.5, float nms_threshold_ = 0.3
...
#endif
  std::cout << "Has GPU:" << this->hasGPU << "  Use GPU:" << useGPU << std::endl; // GPU自检
                    ...
                    float score_threshold, float nms_threshold)
{
  ...
  for (const auto &head_info : this->non_postprocess_heads_info)
  {
    ...
    ncnn::Mat dis_pred;
    ncnn::Mat cls_pred;
    if (head_info.dis_layer.empty())
    {
      std::cout << "dis_layer is empty" << std::endl;
    }
    if (head_info.cls_layer.empty())
    {
      std::cout << "cls_layer is empty" << std::endl;
    }
