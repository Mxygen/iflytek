...
pudding::Pudding yaw_data;

ros::Publisher pcl_pub_test;

const double MIN_RANGE = 0.3; // ��С����
const double MAX_RANGE = 2.0; // ������
double ANG_RANGE = 35;        // �Ƕ����� ��40��
...
void pointCloudCallback(const sensor_msgs::PointCloud2 &cloud_msg)
{
    ...
    if (flag_todeal == 2) // 向右变大
    {
        ...
        yaw_data.fitting_yaw = std::atan2(-coefficients->values[3], coefficients->values[4]);
        if (yaw_data.fitting_yaw > M_PI / 2)
        {
            yaw_data.fitting_yaw = yaw_data.fitting_yaw - M_PI;
        }
        else if (yaw_data.fitting_yaw < -M_PI / 2)
        {
            yaw_data.fitting_yaw = yaw_data.fitting_yaw + M_PI;
        }
        yaw_data.temp_dis = std::sqrt(coefficients->values[0] * coefficients->values[0] + coefficients->values[1] * coefficients->values[1]);
...
int main(int argc, char *argv[])
{
    ...
    yaw_pub = nh.advertise<pudding::Pudding>("/yaw_fromscan", 1);
